...
1. 建立一個header節點作為頭節點
2. 走訪串列時遇到header代表走完所有資料節點。 
3. Begin()回傳第一個資料節點位置，若為空串列Begin() == End()。
4. 串列維持次方由大到小遞減
...
| 測試案例 | 輸入參數 $A$ $B$ $Evaluate$ | 實際輸出A+B |實際輸出A-B |實際輸出A*B |實際輸出Evaluate|
|----------|---------------|----------|----------|----------|----------|
| 測試一   | $A = 3 3 5 -2 2 7 0$ $B = 2 1 1 1 0$ $2$ | 3x^5-2x^2+1x^1+8x^0 | 3x^5-2x^2-1x^1+6x^0 |3x^6+3x^5-2x^3-2x^2+7x^1+7x^0|95|
| 測試二   | $A = 2 3 2 -3 2$ $B = 1 3 2$ $3$ | 3x^2 | -3x^2 |0|0|
